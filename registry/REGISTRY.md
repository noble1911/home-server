...
| **pet-esp / Little Meadow** | `butler/api/routes/pet.py` + shared `~/esp-gateway` | `~/IdeaProjects/pet-esp` | Child's pixel pet: hold-to-talk, live state per turn, dedicated `pet-meadow-3cdc756e3104` identity + pgvector memory. Reuses :8770, :8000 and :8880; no additional service/port. |
...
|---|---|---|
| **Jellyfin** | `8096` | `/Applications/Jellyfin.app` (10.11.11 arm64, upgraded 2026-09-06; rollback copy + data backup in `/Volumes/HomeServer/Backups/jellyfin/`) — runs natively (uses Apple VideoToolbox for HW transcode, which the Linux container can't). Reach from containers via `host.docker.internal:8096`. Container retired 2026-05-25. |
