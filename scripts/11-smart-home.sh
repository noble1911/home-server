...
echo ""
echo "  5. In Cloudflare, add public hostname routes using Docker container names:"
echo "     ha.example.com        → http://homeassistant:8123"
echo "     butler.example.com    → http://butler-app:80"
echo "     butler-api.example.com→ http://butler-api:8000"
echo "     jellyfin.example.com  → http://jellyfin:8096"
echo "     photos.example.com    → http://immich-server:2283"
echo "     books.example.com     → http://audiobookshelf:80"
echo "     files.example.com     → http://nextcloud:80"
echo "     lazylibrarian.example.com → http://lazylibrarian:5299"
echo ""
echo "     IMPORTANT: Use container names (not localhost) because cloudflared"
echo "     runs inside Docker where localhost refers to the container itself."
echo ""
