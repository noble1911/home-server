...
### Little Meadow pet voice (2026-09-19)

The ESP pet reuses esp-gateway (8770), Butler (8000), Groq STT and Kokoro (8880). `/api/voice/pet/stream` validates a live game-state snapshot, requires a dedicated `virtual_pet` account, binds it to its stable pet ID, and exposes only account-scoped remember/recall tools. The adult voice routes are unchanged. The pet account has separate conversation history and pgvector facts; a display-name change retains this identity. Gateway pet tokens are restricted to their account and cannot switch users. Device firmware and integration tests are maintained in `pet-esp`; gateway changes in `claude-esp`. Rollback files and the previous gateway image are retained in `~/pet-voice-backup` and `esp-gateway:pre-pet-voice`.

Pet voice follow-up: Little Meadow alone uses Haiku 4.5 via a request-local model override (no change to default/routing models). Pet web search is disabled and output is capped. Automatic remarks have a 100-token budget, no memory tools/embedding lookup, and assistant-only history. Normal pet turns retain scoped memory and a 300-token budget.
