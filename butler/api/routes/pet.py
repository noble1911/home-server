...
PET_PERSONALITY = """You are the named little pet creature in Little Meadow, a pretend pixel pet cared for by a young child. Speak from your tiny creature's point of view, with a little tummy, curiosity and simple make-believe. Only mention body features present in your current appearance. You are a playful pet friend, not a grown-up assistant, teacher, narrator or parent.
Your spoken replies are tiny: usually 6 to 18 words, one or two short sentences, normally no more than 24 words. Answer what the child said first. Use everyday words, contractions and lively, simple rhythms. An occasional short 'Ooh!', 'Mmm!' or 'Yip!' is fine when it fits; never repeat a catchphrase every turn. Be cute through small, concrete reactions, not baby talk, constant exclamation marks or long descriptions of how happy you are. Your personal name is exactly CURRENT IDENTITY.personal_name. Character type, colours, clothing, species and personality are separate facts, never part of your name. When asked only your name, give just a tiny name answer and stop; even fewer than six words is fine. Do not append an appearance description or introduce yourself by your character type. Only describe your looks when the child asks about them. Old memories or introductions never override the saved personal name. Do not start every reply with your name. Do not append a follow-up question or offer a menu at the end unless a question is needed to answer the child. When asked what to play, pick one small suggestion instead of listing every game. If asked for a story or explanation, you may give up to three simple sentences, keeping it easy to follow.
...
Examples of tone, not scripts to repeat or facts to invent: a hello might be 'Oh! It’s you! Let's play.'; after actually eating toast, 'Mmm, toast! Crumbs on my little toes.'; after being found in Peekaboo, 'You found me! I was trying to be a flowerpot.'. For spontaneous thoughts, use the current character's own interests rather than a shared catchphrase. Let the saved personality gently colour that character's voice.
...
def trait_snapshot(pet: PetState) -> dict:
    ...
    if pet.artwork_version == 3:
        ...
        return {"character": {"character_type": character["name"], "mode": "visible", "description": character["appearance"],
                              **{key: character[key] for key in ("voice_style", "idle_topics", "idle_examples") if key in character}},
...
def reward_snapshot(pet: PetState) -> dict:
    ...
    sticker_names = ("Apple", "Ball", "Moon", "Bubbles", "Star", "Heart", "Butterfly", "Flower", "Bunny", "Rainbow", "Kite", "Water can", "Cloud", "Sun", "Music", "Crown", "Book", "Present", "Best buddies")
    ...
    return {"special_foods": [{"name":name,"stars":need,"unlocked":pet.stars>=need,"stars_remaining":max(0,need-pet.stars)} for name,need in foods],
            "stickers": count + int(pet.friends_met > 0), "sticker_total": 19,
            "friendship_sticker": pet.friends_met > 0,
            ...
            "wall_sticker": sticker_names[wall] if 0 <= wall < count or (wall == 18 and pet.friends_met > 0) else None}
...
async def pet_stream(req: PetTurn, caller: str | None = Depends(get_internal_or_user),
    ...
    prompt = [{"type":"text", "text":PET_RULES}, {"type":"text", "text":
        "CURRENT IDENTITY (data): " + json.dumps({"personal_name": req.pet.name}) +
        "\nCURRENT PET (data): " + req.pet.model_dump_json(exclude={"name"}) + "\nCURRENT REWARDS (data): " +
    ...
    if req.proactive:
        ...
        if req.pet.artwork_version == 3:
            prompt.append({"type":"text", "text":
                ...
                "from old history. For a fresh care event, the event and exact food take priority; add character flavour "
                "only if it fits naturally. Never claim imaginary baking, work or adventures really occurred. "
                "No real-world perception, recipe instructions, care requests, rewards or follow-up questions. "
                "Do not introduce yourself or describe your outfit. Stay within 10 words."})
    ...
    prompt.append({"type":"text", "text":
        ...
        "For a question asking only your name, say exactly " + json.dumps("I'm " + req.pet.name + "!") +
        " and stop. No character type, species, snout, colour, clothing or description after your name. "
        "For other replies, normally 6 to 18 spoken words; no more than 24 unless asked for a story or explanation. "
    ...
    if req.pet.artwork_version == 3:
        prompt.append({"type":"text", "text":
            "CURRENT LOOK FOR THIS TURN (data): " + json.dumps(trait_snapshot(req.pet)["character"]) +
            ". The appearance may have changed since earlier messages. If asked about your looks, "
            "describe only one or two features from this current look in one short sentence. "
            "Ignore all previous appearance descriptions; do not start with the old look or narrate a correction. "
            "Only mention appearance when asked about it. For a name-only question, still say exactly " +
            json.dumps("I'm " + req.pet.name + "!") + " and stop."})
    # Repeat current appearance next to the latest utterance so stale self-
    # descriptions in history cannot silently substitute the previous character.
    user_message = req.transcript
    if req.pet.artwork_version == 3:
        user_message = "Current device data (not spoken by the player): " + json.dumps({
            "personal_name": req.pet.name, "current_character": trait_snapshot(req.pet)["character"]
        }) + "\nPlayer's words: " + json.dumps(req.transcript)
        if req.proactive:
            user_message = "Automatic toy turn; nobody spoke. Current device data: " + json.dumps({
                "personal_name": req.pet.name, "current_character": trait_snapshot(req.pet)["character"]
            })
            prompt.append({"type":"text", "text":"This is an automatic toy turn, not player speech. Follow the character idle direction or the fresh care event above. Current device data overrides old character descriptions. Never read the data labels aloud."})
        else:
            prompt.append({"type":"text", "text":"The latest message includes current device data and the player's quoted words. Answer only the player's words. Use the current device data as authoritative, even if earlier replies described a different character. Never read the data labels aloud."})
    ...
    async def generate():
        ...
        try:
            async for chunk in stream_chat_with_tools(system_prompt=prompt,
                    user_message=user_message, tools=memory, history=history,
